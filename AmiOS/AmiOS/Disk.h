...
struct FAT_Clustor
{
	FAT_Clustor_Record clustor_record; // Clustor ID Record.
...
struct FileSystem
{
	...
	FAT_Clustor getClustor(int16_t clustor_num);
	RESULT writeClustor(int16_t clustor_num, const int8_t * data_array);
	RESULT setClustorRecord(int16_t clustor_num,FAT_Clustor_Record clustor_record);
	RESULT loadTable(int8_t boot_drv);
	RESULT saveTable();
	FAT_File getFile(String file_nm);
	byte readByte(String file_nm,int32_t byte_pos);
	RESULT createFile(FAT_File file_handle);
